...
/** 拷贝 发送对象 的属性到 接收对象 */
export function funcForIn<S, E extends keyof S>(send: S, receiver, exclude?: E | E[]) {
    if (!send) throw 'send is undefined'
    const selfExclude = Array.isArray(exclude) ? exclude : new Array(exclude || '')
    for (const key in send) {
        if (!selfExclude.includes(key) && Object.prototype.hasOwnProperty.call(send, key)) {
